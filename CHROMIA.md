...
### ✅ Реализовано
- **Воспроизведение:** локальные файлы, очередь с shuffle/repeat (off/queue/track), seek, громкость, удалённые стримы (скачивание в кеш через yt-dlp)
- **Библиотека:** сканирование папок, SQLite-хранилище, метадата + обложки (lofty), поиск, плейлисты и история (DB API), счётчик прослушиваний
...
- **Эквалайзер:** 10 полос, пресеты (Flat, Bass Boost, Vocal, Treble Boost, Rock, Jazz)
...
### 🔜 Планируется
- **Воспроизведение:** gapless playback, ReplayGain, crossfade, визуализатор
- **Эквалайзер:** реальный DSP (biquad-цепочка как rodio Source) вместо модели
- **Библиотека:** браузер по артистам/альбомам/жанрам, плейлисты в UI, история в UI, авто-обновление при изменении папок (watch)
- **Стримы:** Spotify через librespot, кеширование стримов с умной очисткой
- **UI:** настройки в GUI, настраиваемые страницы, режим редактирования лейаута, workspaces, состояния виджетов, drag-and-drop по слотам, мини-плеер, блюр обложки за окном, анимации смены цвета, Glass UI режим
...
- Эквалайзер DSP (biquad), ReplayGain, crossfade
- Настройки в GUI
- Браузер библиотеки: артисты/альбомы/жанры, плейлисты в UI
- Авто-обновление библиотеки (watch), кеш-менеджер стримов
